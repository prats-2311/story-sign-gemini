...
function App() {
  ...
  const { isConnected, messages, connect, disconnect, sendMessage, startAudioStream, stopAudioStream, startVideoStream, stopVideoStream } = useGeminiLive({ 
      mode: 'RECONNECT', 
      detectPose 
  }); 
  ...
  const handleAnalyzeSession = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/analyze_session', { // note: need proxy or full URL if CORS not set
        ...
        body: JSON.stringify({ 
            transcript,
            pose_summary: "Session included shoulder flexion exercises." 
        })
      });
